    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> Add(UserAddDto userAddDto)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    toast.AddSuccessToastMessage(Messages.Users.Add(userAddDto.Email), new ToastrOptions { Title = "İşlem Başarılı" });
            ...
            if (user != null)
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (validation.IsValid)
                    {
                        ...
                        if (result.Succeeded)
                        {
                            toast.AddSuccessToastMessage(Messages.Users.Update(userUpdateDto.Email), new ToastrOptions { Title = "İşlem Başarılı" });
                            return RedirectToAction("Index", "User", new { Area = "Admin" });
                        }
                        else
                        {
        ...
        public async Task<IActionResult> Delete(Guid userId)
        {
            ...
            {
                toast.AddSuccessToastMessage(Messages.Users.Delete(result.email), new ToastrOptions { Title = "İşlem Başarılı" });
                return RedirectToAction("Index", "User", new { Area = "Admin" });
            }
            else
            {
